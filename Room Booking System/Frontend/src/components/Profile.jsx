...
function Profile(props) {
  ...
  props.chunkedData.map((row)=>{
    row.filter(card=>card.email===props.email).map((data)=>{
      tempArr.push(data)
    })
  })
  ...
  return (
    ...
    <div id='profile-container'>
        ...
        <div id='profile-details'>
            <div className='profile-details-div'><span id='profile-name'>{props.name}</span>
            </div>

            <div className='profile-details-div'><span id='profile-email'>{props.email}</span>

            </div>
            <button onClick = {logoutclickHandler} id='logout-btn'>Logout</button>
        </div>
    </div>
